...
Pipes are ways to transform template values. For now, only the `raw` pipe is supported. In the future, it may be
possible
to provide custom pipes on application level. If you are a developer, you are welcome to open a PR.

- `raw`: with the raw pipe, values are not checked for xss attacks. As a consequence, the html is not stripped, which
  may
